...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#080b10] text-slate-200">
    ...
    <div className="lg:pl-60"><header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-white/8 bg-[#080b10]/90 px-4 backdrop-blur md:px-7"><div className="lg:hidden text-sm font-semibold">TRADING COPILOT</div><div className="hidden font-mono text-[10px] tracking-[.16em] text-slate-500 lg:block">REGIME → LOCATION → REACTION → EXECUTION → REVIEW</div><div className="flex items-center gap-2 font-mono text-[10px] text-slate-500"><span className="size-1.5 rounded-full bg-emerald-400"/>READ ONLY</div></header><nav className="flex gap-1 overflow-x-auto border-b border-white/8 bg-[#0b0f15] p-2 lg:hidden">{links.slice(0,5).map(([href,name])=><Link key={href} href={href} className={`shrink-0 rounded px-3 py-2 text-xs ${path===href?"bg-cyan-400/10 text-cyan-200":"text-slate-500"}`}>{name}</Link>)}</nav><main className="p-4 md:p-7">{children}</main></div>
  </div>;
}
