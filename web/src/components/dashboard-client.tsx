...
type Data = { account: Account; status: AccountStatus; portfolio: Portfolio; live: LiveStatus; coaches: Record<string, Coach | null> };
export function DashboardClient() {
  const loader = useCallback(async (): Promise<Data> => {
    const [account,status,portfolio,live] = await Promise.all([api.getAccount(),api.getAccountStatus(),api.getPortfolio(),api.getLiveStatus()]);
    ...
    return { account,status,portfolio,live,coaches:Object.fromEntries(coachResults) };
  }, []);
