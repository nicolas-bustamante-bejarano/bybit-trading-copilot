...
export function DashboardClient() {
  ...
  return <>
    ...
    {data && <div className="space-y-5">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4 xl:grid-cols-7"><Metric label="ACCOUNT EQUITY" value={money(data.account.equity_usdt)}/><Metric label="AVAILABLE" value={money(data.account.available_balance_usdt)}/><Metric label="KNOWN RISK" value={money(data.portfolio.known_structural_risk_usdt)}/><Metric label="KNOWN RISK %" value={percent(data.portfolio.known_structural_risk_pct)}/><Metric label="TOTAL RISK" value={data.portfolio.risk_policy_status}/><Metric label="BYBIT PRIVATE" value={data.status.enabled && data.status.credentials_configured ? "CONNECTED" : "DEGRADED"} tone={data.status.enabled ? "text-emerald-300" : "text-amber-300"}/><Metric label="LIVE STREAM" value={data.live.enabled ? "ENABLED" : "OFFLINE"} tone={data.live.enabled ? "text-emerald-300" : "text-slate-400"}/></div>
