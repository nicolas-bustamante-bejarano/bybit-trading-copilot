  ...
  return <>
    ...
    <div className="grid gap-5 xl:grid-cols-[1.05fr_.95fr]">
      <Panel title="Current candidates" kicker={`${ordered.length} VISIBLE`}>
        ...
        {setups.data?.length ? ordered.length ? <CandidateList setups={ordered} triggers={triggerBySetup} selectedId={selectedId} onSelect={setSelectedId}/> : <EmptyState title="No candidates match these filters" detail="Adjust the symbol, setup, or status filter."/> : <EmptyState title={watchlist.data?.length ? "Scanner has not persisted a candidate observation yet." : "Add a market to begin scanning."} detail={watchlist.data?.length ? "Observations will appear after a successful scan cycle." : "Use the watchlist configuration below."}/>}
      </Panel>
      <div className="space-y-5"><CandidateDetail setup={selected} trigger={selectedTrigger}/><TransitionHistory setup={selected}/><TriggerTransitionHistory trigger={selectedTrigger}/></div>
    </div>
...
function CandidateList({ setups, triggers, selectedId, onSelect }: { setups: ScannerSetup[]; triggers: Map<string, TriggerCurrent>; selectedId: string | null; onSelect: (id: string) => void }) {
  return <div className="overflow-hidden rounded border border-white/8"><div className="hidden grid-cols-[.7fr_1.2fr_.4fr_1fr_.75fr_.6fr_.65fr_1fr_.55fr_.65fr] gap-2 border-b border-white/8 bg-black/15 px-3 py-2 font-mono text-[8px] tracking-wider text-slate-600 md:grid"><span>SYMBOL</span><span>SETUP</span><span>SIDE</span><span>STATUS</span><span>LTF TRIGGER</span><span>DATA</span><span>PRICE</span><span>LOCATION / STRUCTURE</span><span>DISTANCE</span><span>UPDATED</span></div>{setups.map((setup) => { const trigger = triggers.get(setup.id); const ltf = setup.status !== "TRIGGER_ARMED" ? null : trigger?.attempt?.state ?? "ARMED"; return <button key={setup.id} onClick={() => onSelect(setup.id)} className={`grid w-full gap-2 border-b border-white/5 p-3 text-left last:border-0 md:grid-cols-[.7fr_1.2fr_.4fr_1fr_.75fr_.6fr_.65fr_1fr_.55fr_.65fr] md:items-center ${selectedId === setup.id ? "bg-cyan-400/[.07]" : "hover:bg-white/[.025]"}`}><div className="font-semibold text-white">{setup.symbol}</div><div className="text-xs text-slate-400">{compactSetup(setup.setup_type)}</div><div className="font-mono text-[10px] text-slate-500">{text(setup.state.side) ?? (setup.setup_type.endsWith("LONG") ? "LONG" : "SHORT")}</div><span className={`w-fit rounded-sm border px-1.5 py-1 font-mono text-[8px] font-semibold ${statusTone(setup.status)}`}>{setup.status}</span>{ltf && ltf !== "ARMED" ? <span className={`w-fit rounded-sm border px-1.5 py-1 font-mono text-[8px] font-semibold ${triggerTone(ltf)}`}>{ltf}</span> : <span className="font-mono text-[9px] text-slate-500">{ltf ?? "—"}</span>}<StatusBadge value={text(setup.state.data_status) ?? "INDETERMINATE"}/><div className="font-mono text-xs text-slate-300">{formatNumber(setup.state.price)}</div><div className="truncate text-xs text-slate-500" title={locationSummary(setup)}>{locationSummary(setup)}</div><div className="font-mono text-xs text-slate-400">{formatNumber(distance(setup), " bps")}</div><div className="text-[10px] text-slate-600">{age(setup.last_evaluated_at ?? setup.updated_at)}</div></button>; })}</div>;
}
...
function CandidateDetail({ setup, trigger: current }: { setup: ScannerSetup | null; trigger: TriggerCurrent | null }) {
  ...
  return <Panel title={`${setup.symbol} · ${compactSetup(setup.setup_type)}`} kicker={`VERSION ${setup.version}`}><div className="space-y-4"><div className="grid gap-2 sm:grid-cols-5">{[["CONTEXT", context], ["LOCATION", location], ["REACTION", reaction], ["TRIGGER", triggerSummary], ["DATA", text(state.data_status) ?? "Indeterminate"]].map(([name, value]) => <div key={name} className="rounded border border-white/7 bg-black/15 p-2.5"><div className="mono-label">{name}</div><div className="mt-1 text-xs leading-5 text-slate-400">{value}</div></div>)}</div>{macro && <MacroEvidence state={state} structure={structure}/>}<EvidenceList title="BLOCKERS" values={blockers}/><EvidenceList title="NEXT" values={next}/>{setup.status === "TRIGGER_ARMED" && <TriggerEvidence current={current}/>}<div className="flex items-center justify-between border-t border-white/7 pt-3 text-[10px] text-slate-600"><span>Evaluated {age(setup.last_evaluated_at)}</span><span className="font-mono text-cyan-300/60">MANUAL EXECUTION ONLY</span></div></div></Panel>;
}
