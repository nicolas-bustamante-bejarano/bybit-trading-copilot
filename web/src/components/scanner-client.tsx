...
import { candidateCounts, defaultScannerTimeframe, filterScannerCandidates, type ScannerTimeframe } from "@/lib/scanner-visuals";
...
export function ScannerClient() {
  ...
  return <>
    ...
    <div className="grid gap-5 xl:grid-cols-[300px_minmax(0,1fr)_390px]">
      ...
      <ScannerEvidenceChart setup={selected} trigger={selectedTrigger} timeframe={chartTimeframe} onTimeframeChange={setChartTimeframe}/>
...
function MonitorStrip({ status, counts }: { status: ReturnType<typeof usePolling<import("@/lib/api/types").ScannerMonitorStatus>>["data"]; counts: ReturnType<typeof candidateCounts> }) {
  const facts = [["TRACKED", counts.tracked], ["ACTIVE", counts.active], ["ARMED", counts.armed], ["LAST SUCCESS", status?.last_success_at ? age(status.last_success_at) : "Never"], ["CYCLE", status?.last_cycle_duration_ms == null ? "—" : `${Math.round(status.last_cycle_duration_ms)} ms`], ["EVALUATED", status?.evaluated_symbols.length ?? "—"], ["FAILED", status?.failed_symbols.length ?? "—"]];
  return <div className="mb-5 grid grid-cols-2 gap-px overflow-hidden rounded border border-white/8 bg-white/8 md:grid-cols-7">{facts.map(([name, value]) => <div key={name} className="bg-[#0d121a] p-3"><div className="mono-label">{name}</div><div className="mt-1 font-mono text-sm text-slate-300">{value}</div></div>)}</div>;
}
